    ...
    async def _upsert_site_info(self, records: list[dict], user_merge_map: dict) -> tuple[int, int]:
        ...
        async with self.repo.pool.acquire() as conn:
            async with conn.cursor() as cur:
                ...
                # Fill every order from its site, not only the domains touched by this
                # run: an incremental sync returns a subset, so orders whose site was
                # synced in an earlier run would otherwise keep a stale theme/category.
                await cur.execute(
                    """UPDATE orders o
                       JOIN site_info s ON LOWER(CASE WHEN LEFT(TRIM(o.product_host), 4)='www.'
                           THEN SUBSTRING(TRIM(o.product_host), 5) ELSE TRIM(o.product_host) END)
