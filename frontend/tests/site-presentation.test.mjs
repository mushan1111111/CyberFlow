...
test('site and indexing analysis use one compact workspace', async () => {
  ...
  assert.match(workspace, /class="site-workspace"/)
  ...
  assert.match(workspace, /row\.login_url/)
  assert.match(workspace, /row\.wp_admin_user/)
  assert.match(workspace, /row\.wp_admin_user_pwd/)
  assert.match(workspace, /passwordText\(row\)/)
  assert.match(workspace, /v-if="isAdmin && dimension === 'site'"/)
  assert.match(workspace, /handleClearAllSites/)
  assert.match(workspace, /订单和商品数据会保留/)
